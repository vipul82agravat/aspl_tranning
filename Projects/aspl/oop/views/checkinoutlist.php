...
<table id="tableID" class="display nowrap">
	...
	<?php
		...
		if(isset($_GET['start_date']) and isset($_GET['end_date'])){
			...
			$data="WHERE ".$table2.".emp_date BETWEEN '".$start_date."' AND '".$end_date."'";
			$selectData="*";
			$getResponse=$getDetails->InnerJoinData($table1,$table2,'id','emp_id',$data,$selectData);
			}else{
			$selectData="*";
				if(isset($_GET['databy']) and $_GET['databy']=='left'){
					$getResponse=$getDetails->leftJoinData($table1,$table2,'id','emp_id',$data=null,$selectData);
				}
				else if(isset($_GET['databy']) and $_GET['databy']=='right'){
					$getResponse=$getDetails->rightJoinData($table1,$table2,'id','emp_id',$data=null,$selectData);
				}else if(isset($_GET['cross']) and $_GET['databy']=='cross'){
					$getResponse=$getDetails->crossJoinData($table1,$table2,'id','emp_id',$data=null,$selectData);
				}else{
